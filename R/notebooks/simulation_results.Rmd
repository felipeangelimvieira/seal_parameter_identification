...
sel.dt <- multisine.dt[(t < 11) & (axis=="x") & (seal == F)]
ggplot(sel.dt) + geom_line(aes(x=t, y=x))

ggplot(multisine.dt[(t < 11) & (axis=="x") & (seal == T)]) + geom_line(aes(x=t, y=x))


config.multisin <- jsonlite::read_json("~/simulation/data/multisin/config.json")
excitation_params <-config.multisin$excitation_params
freqs <- excitation_params$frequencies %>% unlist
phases <- excitation_params$phases %>% unlist
amplitudes <- excitation_params$amplitudes %>% unlist


ones.matrix <- ones(nrow(sel.dt), length(phases))



S <- sin(t(t(ones.matrix) * freqs * sel.dt[, t])*2*pi + t(t(ones.matrix) * phases))

plot(S[1:1000, 3])
inv(t(S) %*% S) %*% t(S) %*% sel.dt[, x]
...
plot.coefs <- function(dt=coef.dt, coefs) {
  gs <- list()
...
g.coefs.k.wo.sweep <- plot.coefs(dt=coef.dt[model != "linreg_sweep"], coefs=c("kxx", "kxy", "kyx", "kyy"))
g.coefs.c.wo.sweep <- plot.coefs(dt=coef.dt[model != "linreg_sweep"], coefs=c("cxx", "cxy", "cyx", "cyy"))
